...
public class IrisExclusiveUniforms {
	...
	public static void addIrisExclusiveUniforms(UniformHolder uniforms) {
		...
		// Lightning strike uniforms for shader integration (world-space position and distance)
		Vector3d zeroVec3 = new Vector3d(0, 0, 0);
		uniforms.uniform3d(UniformUpdateFrequency.PER_TICK, "lightningStrikePos", () -> {
			if (Minecraft.getInstance().level != null) {
		...
		uniforms.uniform1f(UniformUpdateFrequency.PER_TICK, "lightningStrikeDistance", () -> {
			if (Minecraft.getInstance().level != null && Minecraft.getInstance().getCameraEntity() != null) {
				return StreamSupport.stream(Minecraft.getInstance().level.entitiesForRendering().spliterator(), false)
					.filter(bolt -> bolt instanceof LightningBolt)
					.findAny()
					.map(bolt -> {
						Vec3 strikePos = bolt.getPosition(Minecraft.getInstance().getDeltaFrameTime());
						Vec3 playerPos = Minecraft.getInstance().getCameraEntity().getEyePosition(CapturedRenderingState.INSTANCE.getTickDelta());
						return (float) strikePos.distanceTo(playerPos);
					})
					.orElse(-1.0f);
			} else {
				return -1.0f;
