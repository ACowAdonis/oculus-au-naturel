...
public class ShadowRenderTargets {
	...
	// Cached copy strategy to avoid per-frame allocations
	...
	public ShadowRenderTargets(WorldRenderingPipeline pipeline, int resolution, PackShadowDirectives shadowDirectives) {
		...
		// Cache the depth copy strategy - shadow depth is never combined with stencil
		this.depthCopyStrategy = DepthCopyStrategy.fastest(false);
	}
	...
	public void copyPreTranslucentDepth() {
		if (translucentDepthDirty) {
			...
			// Use cached strategy instead of creating new one each frame
			depthCopyStrategy.copy(depthSourceFb, mainDepth.getTextureId(), noTranslucentsDestFb, noTranslucents.getTextureId(),
