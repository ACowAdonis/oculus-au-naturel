...
public class ShadowRenderer {
	...
	private final List<BlockEntity> reusableBlockEntityList = new ArrayList<>();
	private final List<Entity> reusableEntityList = new ArrayList<>(64);
	// Reusable BoxCuller for block entity culling (recreated only when distance changes)
	private BoxCuller blockEntityCuller;
	private double lastBlockEntityCullingDistance = -1;
	// Cached objects for frustum creation to avoid per-frame allocations
	private final CelestialUniforms cachedCelestialUniforms;
	private final Vector3f reusableShadowLightVector = new Vector3f();
	// Cached stateless frustums
	private NonCullingFrustum cachedNonCullingFrustum;
	private CullEverythingFrustum cachedCullEverythingFrustum;
	...
	public ShadowRenderer(ProgramSource shadow, PackDirectives directives,
		...
		// Cache CelestialUniforms since sunPathRotation is constant per shader pack
		this.cachedCelestialUniforms = new CelestialUniforms(sunPathRotation);
	...
	private FrustumHolder createShadowFrustum(float renderMultiplier, FrustumHolder holder) {
		...
		if ((packCullingState == ShadowCullState.DEFAULT && packHasVoxelization) || packCullingState == ShadowCullState.DISTANCE) {
			...
			if (distance <= 0 || distance > Minecraft.getInstance().options.getEffectiveRenderDistance() * 16) {
				...
				// Reuse cached NonCullingFrustum since it's stateless
				if (cachedNonCullingFrustum == null) {
					cachedNonCullingFrustum = new NonCullingFrustum();
				}
				return holder.setInfo(cachedNonCullingFrustum, distanceInfo, cullingInfo);
			} else {
			...
			if (distance >= Minecraft.getInstance().options.getEffectiveRenderDistance() * 16 && !isReversed) {
				...
				if (distance == 0.0 && !isReversed) {
					...
					// Reuse cached CullEverythingFrustum since it's stateless
					if (cachedCullEverythingFrustum == null) {
						cachedCullEverythingFrustum = new CullEverythingFrustum();
					}
					holder.setInfo(cachedCullEverythingFrustum, distanceInfo, cullingInfo);
				}
			...
			// Use cached CelestialUniforms and reusable vectors to avoid per-frame allocations
			Vector4f shadowLightPosition = cachedCelestialUniforms.getShadowLightPositionInWorldSpace();
			reusableShadowLightVector.set(shadowLightPosition.x(), shadowLightPosition.y(), shadowLightPosition.z());
			reusableShadowLightVector.normalize();

			if (isReversed) {
				return holder.setInfo(new ReversedAdvancedShadowCullingFrustum(CapturedRenderingState.INSTANCE.getGbufferModelView(),
					(shouldRenderDH && DHCompat.hasRenderingEnabled()) ? DHCompat.getProjection() : CapturedRenderingState.INSTANCE.getGbufferProjection(), reusableShadowLightVector, boxCuller, new BoxCuller(halfPlaneLength * renderMultiplier)), distanceInfo, cullingInfo);
			} else {
				return holder.setInfo(new AdvancedShadowCullingFrustum(CapturedRenderingState.INSTANCE.getGbufferModelView(),
					(shouldRenderDH && DHCompat.hasRenderingEnabled()) ? DHCompat.getProjection() : CapturedRenderingState.INSTANCE.getGbufferProjection(), reusableShadowLightVector, boxCuller), distanceInfo, cullingInfo);
			}
		}
	...
	public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
		...
		// Reuse the block entity list instead of allocating a new one each frame
		reusableBlockEntityList.clear();
		visibleBlockEntities = reusableBlockEntityList;
	...
	public int renderBlockEntities(MultiBufferSource.BufferSource bufferSource, PoseStack modelView, Camera camera, double cameraX, double cameraY, double cameraZ, float tickDelta, boolean hasEntityFrustum, boolean lightsOnly) {
		...
		if (hasEntityFrustum) {
			// Reuse the BoxCuller, only recreating when the culling distance changes
			double cullingDistance = halfPlaneLength * (renderDistanceMultiplier * entityShadowDistanceMultiplier);
			if (blockEntityCuller == null || lastBlockEntityCullingDistance != cullingDistance) {
				blockEntityCuller = new BoxCuller(cullingDistance);
				lastBlockEntityCullingDistance = cullingDistance;
			}
			blockEntityCuller.setPosition(cameraX, cameraY, cameraZ);
		}
		...
		for (BlockEntity entity : visibleBlockEntities) {
			...
			if (hasEntityFrustum) {
				if (blockEntityCuller.isCulled(pos.getX() - 1, pos.getY() - 1, pos.getZ() - 1, pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1)) {
					continue;
				}
			}
			modelView.pushPose();
	...
	private int renderEntities(LevelRendererAccessor levelRenderer, EntityRenderDispatcher dispatcher, MultiBufferSource.BufferSource bufferSource, PoseStack modelView, float tickDelta, Frustum frustum, double cameraX, double cameraY, double cameraZ) {
		...
		// Reuse the entity list instead of allocating a new one each frame
		...
		reusableEntityList.sort(Comparator.comparingInt(entity -> entity.getType().hashCode()));

		levelRenderer.getLevel().getProfiler().popPush("build entity geometry");

		for (Entity entity : reusableEntityList) {
		...
		return reusableEntityList.size();
	}
